...
import { Calendar, Filter, ArrowUpDown, ArrowUp, ArrowDown, BarChart3 } from "lucide-react";
...
export const DashboardTable = ({ className }: DashboardTableProps) => {
  ...
  const applyFilters = (requests: DashboardRequest[]): DashboardRequest[] => {
  ...
  const applySorting = (requests: DashboardRequest[]): DashboardRequest[] => {
  ...
  const filteredAndSortedRequests = applySorting(applyFilters(mockDashboardRequestsData));
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl">All Requests History</CardTitle>
                <p className="text-muted-foreground">Complete overview of all recipe requests and their current status</p>
              </div>
            </div>
            <Button
                ...
                <div className="space-y-3">
                  ...
                  <Select value={filters.status} onValueChange={(value) => setFilters(prev => ({ ...prev, status: value }))}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="REQUEST CREATED,PENDING ON CHEF">Pending on Chef</SelectItem>
                      <SelectItem value="APPROVAL PENDING ON CATEGORY">Pending on Category</SelectItem>
                      <SelectItem value="APPROVAL PENDING ON SC">Pending on SC</SelectItem>
                      <SelectItem value="APPROVAL PENDING ON QUALITY">Pending on Quality</SelectItem>
                      <SelectItem value="APPROVAL PENDING ON FINANCE">Pending on Finance</SelectItem>
                      <SelectItem value="APPROVED BY ALL, PENDING ON MDM">Pending on MDM</SelectItem>
                      <SelectItem value="ACKNOWLEDGED BY MDM, CHANGES PENDING">Changes Pending</SelectItem>
                      <SelectItem value="DONE BY MDM, AWAITING ROLLOUT">Awaiting Rollout</SelectItem>
